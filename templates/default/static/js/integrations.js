	...
	curInst.$loadingContainer.fadeOut( 400, function () {
		...
		if ( 'error' === accountResponse['responseType'] ) {
			...
			if ( campaignResponse.hasOwnProperty('responseType') && 'error' === campaignResponse['responseType'] ) {
				...
				var campaignSelected = ( curInst.selectedCampaignId == campaignResponse['data']['id'] ) ? 'selected="selected"' : '';
				campaignSelectHTML += '<option value="' + campaignResponse['data']['id'] + '" ' + campaignSelected + '>' + campaignResponse['data']['name'] + '</option>';
